...
### Phase D13 D7 後處理優化與 Per-class 閾值尋優（完成並拒絕）

- **尋優演算法**：以 D7 作為唯一模型，在 validation 集合（8064 windows 中取 8x6=48 validation windows）上，執行**逐類別座標上升搜尋 (Coordinate Ascent Search)**。
- **最佳解碼閾值組合**：
  - 大鼓 (KD): `0.60`
  - 小鼓 (SD): `0.60`
  - 踩镲 (HH): `0.60`
  ...
  - 疊鈸 (RIDE): `0.55`
- **Validation 指標提振效果**：
  - **Macro F1**：從 `0.4601` 提升至 **`0.4798` (+0.0197)**。
  - **大鼓 KD F1**：從 `0.7046` 提升至 **`0.7294` (+0.0249)**。
  - **小鼓 SD F1**：從 `0.7151` 提升至 **`0.7196` (+0.0045)**。
  - **踩镲 HH F1**：從 `0.5294` 提升至 **`0.5639` (+0.0346)**。
  - **中音鼓 TOM F1**：從 `0.3125` 提升至 **`0.3449` (+0.0324)**。
  - **碎鈸 CRASH F1**：從 `0.1390` 提升至 **`0.1503` (+0.0113)**。
  - **疊鈸 RIDE F1**：從 `0.3600` 提升至 **`0.3704` (+0.0104)**。
- **實體獨立驗收對照結果**：
  - **Blind User Tests 物理音符對照**：大鼓 KD 假陽性大幅減少，在 3 首歌曲中實現物理音符 100% 完璧匹配。小鼓 SD 假陽性大砍近一半！
  - **E-GMD Round 4 Validation 的 Strong-hit 測試**：
    - 優化前預設閾值 (All 0.50)：**28/30 pass**。
    - 優化後最優閾值：**27/30 pass**。
    - **退步點分析**：由於將大鼓 KD、踩镲 HH 閾值拉高到 `0.60`，在部分動態大的實體音軌中，偏輕的音符落入漏檢（FN 增加），導致 HH strong F1 從 `0.95` 跌落至 `0.81`。
- **安全防線判定**：
  - 雖然在 validation 上獲得了大幅的 Macro F1 增長，但由於在 E-GMD Round 4 實體歌曲的強音 (strong hits) 上發生了 `28/30` -> `27/30` 的反向退步，且 Blind 測試中 HH 出現了漏檢退步，違反了「Blind 與 Round 4 都沒有反向退步」的晉級規則。
- **本期判定**：D13 實驗失敗，不予晉級。繼續保留 D7 預設 0.50 作為系統預設值。
