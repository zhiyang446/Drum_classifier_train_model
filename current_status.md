...
- Next-coverage audit: a read-only sample of the first 500 E-GMD `split=test` MIDI files found excluded pitch `22` in `14,063` events / `287` files and pitch `26` in `1,840` events / `280` files. These dominate the excluded set and should be treated as the next HH-articulation mapping question before adding ride/tom classes.
...
- Current next direction: do not tune thresholds or add broad tempo aliases for the completed Round4 gate. The next validation step should validate whether E-GMD pitch `22` and `26` belong in reusable HH coverage before any new drum-class training.
