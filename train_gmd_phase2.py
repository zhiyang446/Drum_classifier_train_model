...
BATCH_SIZE = 16
EPOCHS = 45 # Standard Phase 2 duration
CHECKPOINT_PATH = 'checkpoint_gmd_phase2.pth'
...
class UniversalCleanDrumDataset(Dataset):
    """
    Dataset for Phase 2: Solo/Clean drum training (No accompaniment mixing, no speed augmentation).
    ...
    def __getitem__(self, idx):
        ...
        if is_gmd:
            ...
            if total_samples > TARGET_SAMPLES:
                start_sample = np.random.randint(0, total_samples - TARGET_SAMPLES) if self.is_training else 0
            else:
                start_sample = 0
            
            drum_slice = load_npy_slice(wave_npy_path, start_sample, TARGET_SAMPLES)
            y_mix = drum_slice
            ...
            end_sec = (start_sample + TARGET_SAMPLES) / SR
            ...
            if self.is_training:
                start_sample = np.random.randint(0, total_samples - TARGET_SAMPLES) if total_samples > TARGET_SAMPLES else 0
            else:
                part = idx % 2
...
def main():
    ...
    print("Initializing Universal Clean Drum Datasets...")
    train_dataset = UniversalCleanDrumDataset(is_training=True)
    val_dataset = UniversalCleanDrumDataset(is_training=False)
    ...
    if os.path.exists('best_drum_model_phase2.pth'):
        print("Loading existing Phase 2 clean weights from best_drum_model_phase2.pth...")
    ...
    optimizer = optim.Adam(model.parameters(), lr=0.001) # Standard training rate
    ...
    if os.path.exists(CHECKPOINT_PATH):
        ...
        try:
            ...
            print(f"Failed to load checkpoint ({e}). Starting Phase 2 GMD training from scratch.")
            
    print("\nStarting Universal Phase 2: Solo/Clean Drum Training...")
    for epoch in range(start_epoch, EPOCHS):
        ...
        beta = 20.0 if epoch < 20 else 10.0 # Phase 2 loss weight scheduling
        ...
        print(f"\n--- Epoch {epoch+1:02d} Validation Results (Clean) ---")
        print(f"Val F1: [KD: {f1s[0]:.3f}, SD: {f1s[1]:.3f}, HH: {f1s[2]:.3f}] | Mean F1: {mean_f1:.3f}")
        print(f"Val Vel RMSE: {rmse:.2f}")
        
        # Save best model to Phase 2 checkpoint
        if mean_f1 > best_val_f1 or (abs(mean_f1 - best_val_f1) < 0.01 and rmse < best_val_rmse):
            best_val_f1 = mean_f1
            best_val_rmse = rmse
            torch.save(model.state_dict(), 'best_drum_model_phase2.pth')
            print(f"  --> Saved new best clean model: best_drum_model_phase2.pth (Mean F1: {mean_f1:.3f}, RMSE: {rmse:.2f})")
    ...
    print(f"\nPhase 2 Clean Training finished. Best Model saved to best_drum_model_phase2.pth")
